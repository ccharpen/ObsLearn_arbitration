function f = LL_function_model6_EM_RL(params,P)

%this function generate loglikelihood of a model inferring the value of each
%token (emulation), but using RL instead of mulplicative (approx. Bayesian)
%inference. 
%Learning rate varies dynamically, on a trial-by-trial basis, depending on
%the magnitude of the prediction error obtained on the previous trial.
...
for t=1:tr_nb
    ...
    if tr_type(t)==1 %only update after observe trials
        ...
        if P(t,3)==1 
            ...
            V(t,:) = [0 0 0] + alpha(t)*(token_ev - [0 0 0]);
        else
            alpha(t) = params(2)*mean(abs(token_ev-V(t-1,:))) + (1-params(2))*alpha(t-1);
            V(t,:) = V(t-1,:) + alpha(t)*(token_ev-V(t-1,:)); %value update 
        end
