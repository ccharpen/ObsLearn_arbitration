function post = LL_function_newMod10_forEM(params,thetamean,thetasigma,P)

%This function describes a model to estimate arbitration
%between 1-trial imitation and emulation.
%Caroline Charpentier - 4/21/2017
...
for t=1:tr_nb
    ...
    if tr_type(t)==1 %observe
        ...
        %Since xBI represents the UNreliability, take minus of these to get reliability
        rBI = 1 - 2*xBI(t); %reliability of BI (now ranges from -1 to +1)
        w(t) = (1+exp(-(params(3) + rBI)))^-1; %do not estimate inverse temperature
